...
import scales, { Scale } from "~/common/scales";
...
export const ScalePicker: Component<{ text: string }> = function (props) {
                      ...
                      <For each={TABS}>
                        {(tab) => (
                          <button
                            ...
                            aria-current={
                              currentTabId() === tab.id ? "page" : undefined
                            }
                          >
                            {tab.name}
                          </button>
...
const MajorScales: Component = function () {
  return (
    <For each={scales}>
      {(scale: Scale) =>
        scale.mode == "major" && (
          <Link href={`/scales/${scale.key}/${scale.mode}`}>{scale.name}</Link>
        )
      }
    </For>
  );
